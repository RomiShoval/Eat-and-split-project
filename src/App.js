...
function Friend({ friend, onSelection, selectedFriend }) {
  ...
  return <li className={isSelcted ? 'selected' : ""}>
    <img src={friend.image} alt={friend.name} />
    <h3>{friend.name}</h3>

    {friend.balance < 0 && <p className="red">Yow owe
      {friend.name} {Math.abs(friend.balance)}</p>}

    {friend.balance > 0 && <p className="green">
      {friend.name} owes you {Math.abs(friend.balance)}</p>}

    {friend.balance === 0 && <p >Yow and
      {friend.name} are even</p>}

    <Button onClick={() => onSelection(friend)}>{isSelcted ? 'Close' : 'Select'}</Button>
  </li>
}
...
function FormSplitBill({ selectedFriend, onSplitBill }) {
  ...
  return <form className="form-split-bill" onSubmit={handleSubmit}>
    ...
    <input type="text" value={paidByUser} onChange={(e) => setPaidByUser(
      Number(e.target.value) > bill ? paidByUser : Number(e.target.value))}></input>
